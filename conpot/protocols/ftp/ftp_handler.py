...
class FTPCommandChannel(FTPHandlerBase):
    ...
    def do_RMD(self, path):
        ...
        try:
            ...
            with self.config.vfs.check_access(path=self.ftp_path(os.path.join(path, '../')),
                                              user=self._uid, perms='w'):
    ...
    def do_RNFR(self, path):
        ...
        try:
            ...
            self.respond(b'500 Operation not permitted.')
        except (AssertionError, KeyError, fs.errors.FSError, FilesystemError, FTPPrivilegeException):
    ...
    def do_STOR(self, file, mode='w'):
        ...
        except (OSError, AssertionError, fs.errors.FSError, FilesystemError, FTPPrivilegeException) as err:
            self._log_err(err)
