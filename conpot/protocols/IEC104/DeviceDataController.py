...
class DeviceDataController(object):
    def __init__(self, template):
        ...
        for category in categories:
            ...
            for register in category:
                ...
                assert (not(categ_id in (1, 2, 30, 45, 58) and val not in (0, 1))),\
                    "Value for obj %s not allowed with datatype %s" % (address, categ_id)
                assert (not(categ_id in (3, 4, 31, 46, 59) and val not in (0, 1, 2, 3))), \
                    "Value for obj %s not allowed with datatype %s" % (address, categ_id)
                assert (not(categ_id in (11, 12, 49, 62) and -32768 >= val >= 32767)), \
                    "Value for obj %s not allowed with datatype %s" % (address, categ_id)
    ...
    def check_registers(self):
        for elem in self.registers:
            ...
            assert (not (rel != "" and rel not in self.registers)), "Relation object doesn't exist"
...
def inro_response(sorted_reg, asdu_type):
    ...
    calls_dict = {'asdu_infobj_1': asdu_infobj_1, 'asdu_infobj_3': asdu_infobj_3,
                  'asdu_infobj_5': asdu_infobj_5, 'asdu_infobj_7': asdu_infobj_7,
                  'asdu_infobj_9': asdu_infobj_9, 'asdu_infobj_11': asdu_infobj_11,
