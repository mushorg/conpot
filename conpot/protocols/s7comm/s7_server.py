...
class S7Server(object):
    def __init__(self, template, template_directory, args):
        ...
        logger.debug('Conpot debug info: S7 SSL/SZL: %s', self.ssl_lists)
        ...
        try:
            while True:
                ...
                if cotp_base_packet.tpdu_type == 0xe0:
                    ...
                    logger.debug('Received COTP Connection Request: dst-ref:%s src-ref:%s dst-tsap:%s src-tsap:%s '
                                 'tpdu-size:%s. (%s)', cotp_cr_request.dst_ref, cotp_cr_request.src_ref,
                    ...
                    if cotp_base_packet.tpdu_type == 0xf0:
                        logger.debug('Received known COTP TPDU: %s. (%s)', cotp_base_packet.tpdu_type,
                                                                                   session.id)
                    ...
                    logger.debug('Received unknown COTP TPDU before handshake: %s', cotp_base_packet.tpdu_type)
    ...
    def start(self, host, port):
        ...
        logger.info('S7Comm server started on: %s', connection)
        self.server.start()
