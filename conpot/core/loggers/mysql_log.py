...
class MySQLlogger(object):
    def __init__(
        self, host, port, db, username, passphrase, logdevice, logsocket, sensorid
    ):
        self.host = host
        self.port = port
        self.db = db
    ...
    def _connect(self):
        try:
            ...
            logger.error(
                "Could not create a stable database connection for logging. Check database and credentials."
            )
    ...
    def log(self, event, retry=1):
        ...
        try:
            ...
            if retry == 0:
                ...
                logger.debug(
                    "Logging failed: Database connection lost. Retrying (%s tries left)...",
                    retry,
                )
                retry -= 1
