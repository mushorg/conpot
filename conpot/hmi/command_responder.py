...
from datetime import datetime       # imported for user defined evals
        ...
        if xml_headers:
            ...
            for header in xml_headers:
                ...
                if header.attrib['name'].lower() == 'date' and self.update_header_date == True:
                    # All HTTP date/time stamps MUST be represented in Greenwich Mean Time (GMT), without exception ( RFC-2616 )
                    self.global_headers.append((header.attrib['name'], time.strftime('%a, %d %b %Y %H:%M:%S GMT', time.gmtime())))
                else:
