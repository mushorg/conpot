...
class S7(object):
    ...
    def request_not_implemented(self):
        raise ParseException(self.protocol, 'request not implemented in honeypot yet.')

    def pack(self):

        if self.pdu_type not in self.pdu_mapping:
            raise AssembleException(self.protocol, 'invalid or unsupported pdu type')
    ...
    def parse(self, packet):
        ...
        try:
            ...
            raise ParseException(self.protocol, 'malformed fixed packet header structure')

        self.magic = int(fixed_header[0])

        if self.magic != 0x32:
            raise ParseException(self.protocol, 'bad magic number, expected 0x32 but got {0}.'.format(self.magic))
    ...
    def request_diagnostics(self):
        ...
        try:
            ...
            raise ParseException(self.protocol, 'malformed SSL/SZL parameter structure')
        ...
        while chunk:
            try:
                ...
                raise ParseException(self.protocol, 'malformed SSL/SZL data structure')
