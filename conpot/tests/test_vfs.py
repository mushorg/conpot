...
class TestFileSystem(unittest.TestCase):
    ...
    def test_chmod(self):
        ...
        self.assertEqual(fs.permissions.Permissions.create(0o500),
                         self.test_vfs.getinfo(path='/data', namespaces=['access']).permissions)
        # No changes made in the actual file system.
        self.assertNotEqual(fs.permissions.Permissions.create(0o500),
                            self.test_vfs.getinfo(path='/data', get_actual=True, namespaces=['access']).permissions)
    ...
    def test_chown(self):
        ...
        # check uid
        self.assertEqual(self.test_vfs.getinfo('/data', namespaces=['access']).uid, 3000)
        ...
        # check file group
        self.assertEqual(self.test_vfs.getinfo('/data', namespaces=['access']).group, 'test_grp')
        self.assertNotEqual(self.test_vfs.getinfo('/data', get_actual=True, namespaces=['access']).group, 'test_grp')
    ...
    def test_mkdir(self):
        ...
        self.assertNotEqual(self.test_vfs.getinfo('/dummy_dir', get_actual=True, namespaces=['access']).permissions,
                            fs.permissions.Permissions.create(0o500))
        # check the usr/grp that created the folder
        self.assertEqual(self.test_vfs.getinfo('/dummy_dir', namespaces=['access']).uid, self.test_vfs.default_uid)
    ...
    def test_snapshot(self):
        self.assertEqual({'date-time', 'snapshot-data'}, set(self.test_vfs.take_snapshot().keys()))
        self.assertEqual(self.test_vfs.take_snapshot()['date-time'], datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    ...
    def test_stat(self):
        ...
        self.assertTrue(all(
            ...
            for i in {'st_uid', 'st_gid', 'st_mode', 'st_atime', 'st_mtime', 'st_mtime_ns', 'st_ctime_ns', 'st_ctime'}
            ))
    ...
    def test_get_permissions(self):
        self.assertEqual(self.test_vfs.getinfo('/data', namespaces=['access']).permissions.as_str(),
...
class TestSubFileSystem(unittest.TestCase):
    ...
    def test_chmod(self):
        ...
        # No changes made in the actual file system.
        self.assertNotEqual(fs.permissions.Permissions.create(0o500),
                            self.test_vfs.getinfo(path='vfs.txt', get_actual=True, namespaces=['access']).permissions)
    ...
    def test_chown(self):
        ...
        # check uid
        self.assertEqual(self.test_vfs.getinfo('vfs.txt', namespaces=['access']).uid, 3000)
        # actual uid shouldn't have changed
        self.assertNotEqual(self.test_vfs.getinfo('vfs.txt', get_actual=True, namespaces=['access']).uid, 3000)

    def test_mkdir(self):
        self.test_vfs.makedir('/dummy', permissions=0o500)
        self.assertFalse(self.test_vfs.access('/dummy', self.test_vfs.default_uid, 'rwx'))
        self.test_vfs.removedir('/dummy')
        self.assertNotIn('/dummy', self.test_vfs._cache.keys())
    ...
    def test_access(self):
        ...
        self.assertEqual(self.test_vfs.getinfo('/vfs.txt', namespaces=['access']).uid, 13)
